...
import {
    Search, ChevronDown, Briefcase, MapPin,
    Github, Calendar, CheckCircle, XCircle,
...
function MessageModal({ app, onClose }: { app: Application; onClose: () => void }) {
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded-full ${app.avatarColor} text-white text-sm font-bold flex items-center justify-center`}>
                            {app.candidateInitials}
                        </div>
                        <div>
                            <h3 className="text-sm font-bold text-gray-900">Message {app.candidateName}</h3>
                            <p className="text-xs text-gray-500">Re: {app.jobTitle}</p>
                        </div>
                    </div>
                ...
                <div className="flex gap-2">
                    <button onClick={onClose} className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Cancel</button>
                    <button onClick={onClose} className="flex-1 px-3 py-2 bg-blue-700 hover:bg-blue-800 text-white rounded-lg text-sm font-semibold">Send Message</button>
                </div>
            </div>
        </div>
    );
...
export default function RecruiterApplicationsPage() {
    ...
    return (
        <div className="space-y-5">
            ...
            {msgTarget && <MessageModal app={msgTarget} onClose={() => { setMsgTarget(null); showToast("Message sent!"); }} />}
