...
function relativeTime(ms: number) {
    ...
    if (diff < 1000 * 60 * 60) return `${Math.round(diff / 60000)}m ago`;
    if (diff < 1000 * 60 * 60 * 24) return `${Math.round(diff / 3600000)}h ago`;
...
function NewMessageModal({ onClose, candidates }: { onClose: () => void; candidates: { id: string; name: string; initials: string; color: string; jobTitle: string }[] }) {
                            ...
                            <div className="mt-1 border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                                {filtered.length === 0 ? (
                                    ...
                                    filtered.map((c) => (
                                        <button
                                            key={c.id}
                                            onClick={() => { setSelected(c.id); setSearch(c.name); }}
                                            className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-blue-50 transition-colors text-sm ${selected === c.id ? "bg-blue-50" : ""}`}
                                        >
                                            ...
                                            {selected === c.id && <Check size={13} className="ml-auto text-blue-600" />}
                                        </button>
                                    ))
                                )}
                            </div>
...
export default function MessagesPage() {
    ...
    // Candidate list for new message modal
    const candidateList = conversations.map((c) => ({
        id: c.candidateId,
